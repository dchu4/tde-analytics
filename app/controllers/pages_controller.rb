...
class PagesController < ApplicationController
  ...
  def index
    ...
    transaction_count = result.collect { |row| row[3] }

    @conversion_rate = "#{((transaction_count.uniq.length.to_f/Visit.where(created_at: 31.days.ago..1.day.ago).count.to_f)*100).round(2)}%"
